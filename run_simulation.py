"""Unified simulation runner for viscoelastic creep analysis.

Usage::

    python run_simulation.py 1D          # 1D analytical (J-body, eq. 3)
    python run_simulation.py 2D          # 2D PorePy, no fracture
    python run_simulation.py 2D_frac     # 2D PorePy, with fracture
...
def print_help():
    ...
    print("=" * 60)
    print("  Viscoelastic Creep Simulation Runner")
    print("=" * 60)
    print()
    print("Usage:")
    ...
    print()
    print("Examples:")
    print("  python run_simulation.py 1D")
    print("  python run_simulation.py 2D")
    print("  python run_simulation.py 2D_frac")
...
def main():
    ...
    if mode == "1D":
        ...
        print("Running 2D simulation (no fracture)...")
        # Import and execute the no-fracture simulation
        import os
        import numpy as np
        import porepy as pp
        ...
        print("=" * 60)
        print("  Viscoelastic creep test -- NO FRACTURE")
        ...
        save_strain_history("_output/strain_no_frac.npz", model.strain_history)
        print("Saved _output/strain_no_frac.npz")

        snap = model._displacement_snapshots.get(FINAL_TIME)
        if snap is not None:
            model.plot_displacement_map(
                snap["u"],
                title="|u| at t = 8 h (no fracture)",
                filepath="_output/displacement_no_frac_8h.png",
            )
        ...
        if len(model.strain_history["times"]) > 0:
            ...
            ax.plot(
                EXPERIMENTAL_DATA_T, EXPERIMENTAL_DATA_EPS,
                "bD", markersize=6, label="Experimental data",
            )
            ax.plot(
                t_1d, eps_1d,
                "k--", linewidth=1.5, label="1D simulation (article)",
            )
            ax.plot(
                t, np.abs(eyy_u) * 100,
                "r-", linewidth=1.5, label="2D simulation (PorePy)",
            )
            ...
            ax.tick_params(width=1.5, direction="in", top=True, right=True)
            fig.tight_layout()
            fig.savefig("_output/strain_eyy_no_frac.png", dpi=300)
            plt.close(fig)
            print("Saved _output/strain_eyy_no_frac.png")

        print("Done.")

    elif mode in ("2D_FRAC", "2DFRAC", "FRAC"):
        print("Running 2D simulation (with fracture)...")
        import os
        import numpy as np
        import porepy as pp
        import matplotlib.pyplot as plt
        ...
        solid = ViscoelasticSolidConstants(
            ...
            lame_lambda2=E2 * NU / ((1.0 + NU) * (1.0 - 2.0 * NU)),
            viscosity=ETA,
            fracture_normal_stiffness=200_000_000_000.0,
            fracture_tangential_stiffness=100_000_000_000.0,
        )
